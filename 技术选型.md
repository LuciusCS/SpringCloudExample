...
2. Auditing 使用 Spring Framework 6 中的
3. 在开发的过程中使用乐观锁
...
7.连接池推荐使用 HikariCP。
   HikariCP 是目前 性能最优 的数据库连接池之一，Spring Boot 2.x/3.x 默认集成。
   Druid 虽然功能丰富（监控），但在云原生环境下，推荐使用 Prometheus + Grafana 进行监控，而不是依赖 Druid 自身的监控页面。
...
## 10、 定时任务
1、Quartz：传统的定时任务框架，基于数据库锁，集群性能较差，不推荐在大规模分布式场景使用。
2、Elastic-Job：基于 Zookeeper，功能强大但维护状态一般。
3、XXL-Job：轻量级分布式任务调度平台，文档齐全，社区活跃，推荐使用。
4、PowerJob：新一代分布式任务调度与计算框架，支持 MapReduce 处理，性能强劲，推荐在需要高性能计算场景使用。
...
1.  **集群模式**：生产环境推荐使用 **Quorum Queues** (仲裁队列)，它是基于 Raft 协议实现的，提供强一致性和高可用性，替代了传统的镜像队列（Mirrored Queues 在 RabbitMQ 4.0 将被移除）。
2.  **流控**：配合 Sentinel 或 RabbitMQ 自身的流控机制。
