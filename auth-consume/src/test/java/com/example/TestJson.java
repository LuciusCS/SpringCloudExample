...
public class TestJson {
    ...
    public static void main(String[] args) throws IOException {
        ...
        FileInputStream fis = new FileInputStream("/Users/eastsoft/Documents/Development/LuciusGithub/JavaProject/SpringCloudExample/auth-consume/src/main/resources/area.json");
        String stringTooLong = IOUtils.toString(fis, Charset.forName("UTF-8"));

        List<Area> area= JSONObject.parseArray(stringTooLong,Area.class);

        Map<String,String>province=new HashMap<>();

        Map<String, Map<String,MapArea>>cities=new HashMap<>();


        MapArea mapArea=new MapArea("123", "a");

//        System.out.println(JSONObject.toJSONString(mapArea));

        int length=area.size();

        for (int i=0;i<length;i++){

            province.put(String.valueOf((area.get(i).value+10000)),area.get(i).label);

            if(null!=area.get(i).children) {

                List<Area> children = area.get(i).children;

                Map<String, MapArea> citiesTmp = new HashMap<>();

                int childrenLength = children.size();

                for (int j = 0; j < childrenLength; j++) {

                    if (null != children.get(j).children) {

                        List<Area> grandSon = children.get(j).children;

                        Map<String, MapArea> citiesTmp1 = new HashMap<>();

                        ///用于表示第三层级
                        int grandSonLength = grandSon.size();
                        for (int k = 0; k < grandSonLength; k++) {
                            citiesTmp1.put(String.valueOf(grandSon.get(k).value), new MapArea(grandSon.get(k).label, "a"));
                        }

                        cities.put(String.valueOf((children.get(j).value+100000)), citiesTmp1);
...
//                    else {
                    if (null != children.get(j).children) {
                        ...
                        citiesTmp.put(String.valueOf((children.get(j).value + 100000)), new MapArea(children.get(j).label, "a"));
                    }else {
                        citiesTmp.put(String.valueOf((children.get(j).value )), new MapArea(children.get(j).label, "a"));

                    }
                      //                    }
                }

                cities.put(String.valueOf((area.get(i).value+10000)), citiesTmp);
            }
...
//    static String jsonInfo="  [{\"value\": 1, \"label\": \"北京\", \"children\": [\n" +
//            "    {\"value\": 1, \"label\": \"东城区\"},\n" +
//            "    {\"value\": 2, \"label\": \"西城区\"},\n" +
//            "    {\"value\": 3, \"label\": \"崇文区\"},\n" +
//            "    {\"value\": 4, \"label\": \"宣武区\"},\n" +
